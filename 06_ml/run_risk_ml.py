...
def _compute_risk(df, mode):
    if mode == "sed_max":
        ...
        # Weighted proxy (normalize before combining if used in dissertation).
...
def main():
    ...
    df = _load_feature_csvs(
        args.features_dir, include_cyclewise=args.include_cyclewise, include_prefixes=args.include_prefix
    )
    df = _compute_risk(df, args.risk_mode)
    inputs = _select_inputs(df)
